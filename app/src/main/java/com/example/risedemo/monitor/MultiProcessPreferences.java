...
/**
 * 多进程获取值，通过android SharedPreferences是不可靠的，通过contentProvider是比较好的选择
 *
 * <p>
 * 1.多进程下传值可以通过aidl注册binder通信来实现；
 * 2.多进程读取值，比如子进程读取主进程更新的值，可以采用：
 * 2.1 本地文件的方式，将值写入到本地文件，每次都可以通过读取文件实现，但该种方式需要IO，所以占用资源
 * 2.2 通过ContentProvider实现，ContentProvider的注册时机很早，然后实现也是数据库；
 * 2.3 通过手写数据库存储实现
 * 2.4 还可以通过socket通过服务端来实现
 * <p/>
 */
